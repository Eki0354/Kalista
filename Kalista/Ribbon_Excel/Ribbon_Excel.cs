...
namespace Kalista
{
    public partial class Ribbon_Excel
    {
        ...
        private void button_tool_locateBK_Click(object sender, RibbonControlEventArgs e)
        {
            if (lastBKFilePath == null || !File.Exists(lastBKFilePath)) return;
            System.Diagnostics.Process.Start("explorer.exe", "/select," + lastBKFilePath);
        }

        private void button_toJSON_Click(object sender, RibbonControlEventArgs e)
        {
            var h = new RoomStatusJSON.RoomStatusJSONHelper(ActWorkbook);
            h.ToJSON();
        }
    }
}
